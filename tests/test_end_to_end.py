        ...
        async with websockets.connect(sender_uri, max_size=10 * 1024 * 1024) as sender_ws:
            ...
            async with websockets.connect(control_uri, max_size=10 * 1024 * 1024) as control_ws:
                ...
                await control_ws.send(
                    '{"type":"swipe","x0":0.1,"y0":0.2,"x1":0.9,"y1":0.8,"duration_ms":250}'
                )

            tap_message = await asyncio.wait_for(sender_ws.recv(), timeout=5)
            swipe_message = await asyncio.wait_for(sender_ws.recv(), timeout=5)
            assert tap_message == '{"type":"tap","x":10,"y":20}'
            assert (
                swipe_message
                == '{"type":"swipe","x0":0.1,"y0":0.2,"x1":0.9,"y1":0.8,"duration_ms":250}'
            )
