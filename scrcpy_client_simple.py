...
class VideoDecoderThread(QThread):
    ...
    def _create_test_frame(self, frame_count):
        ...
        color = int((frame_count * 7) % 256)
        img.fill(QColor(color, color, color))
        
        painter = QPainter(img)
        painter.setPen(QColor(255, 255, 255))
        font_size = 24
        painter.setFont(painter.font())
        painter.drawText(50, height//2, f'Frame: {frame_count}')
        painter.end()
        ...
        if self.scrcpy.start_server():
            ...
            if self.adb.forward_port(it.text(), 27183, 27183):
                ...
                try:
                    ...
                    self.mouse_status.setText('Mouse Control: Active (Click canvas to control phone)')
...
def main():
    ...
    app.setStyle('Fusion')
    app.setApplicationName("Scrcpy Client")
    win = ScrcpyClientGUI()
    win.show()
    sys.exit(app.exec_())
