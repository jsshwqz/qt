#!/usr/bin/env python3
"""
Scrcpy Client - Final Working Version
使用 3.3.3 版本启动 Scrcpy Server
"""
import sys
import time
import socket
...
class SimpleAdbManager:
    ...
    def start_server(self):
        """启动 ADB 服务"""
        try:
            subprocess.run([self.adb, "kill-server"], capture_output=True, timeout=3, creationflags=0x08000000)
            time.sleep(1)
            subprocess.run([self.adb, "start-server"], capture_output=True, timeout=5, creationflags=0x08000000)
            time.sleep(1)
            return True
    ...
    def list_devices(self):
        """列出设备"""
        try:
            result = subprocess.run([self.adb, "devices"], capture_output=True, text=True, timeout=5, creationflags=0x08000000)
            devices = []
...
class SimpleScrcpyServer:
    ...
    def start(self):
        ...
        try:
            ...
            # 启动服务 - 使用版本 3.3.3
            logger.info("[*] Starting Scrcpy Server (v3.3.3)...")
            cmd = [
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.frame_count += 1
                logger.debug(f"Frame {self.frame_count}: size={size}")
...
class ScrcpyGUI(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Scrcpy Client - v3.3.3')
